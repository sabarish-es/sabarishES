...
const projects = [
  ...
  { title: 'ClassValut', description: 'A responsive student and faculty administration portal for managing academic records and workflows.', tech: 'PHP · MySQL · JavaScript', href: '#contact', className: 'project-4' },
]
...
export default function Page() {
  ...
  return (
    <main className="portfolio-shell min-h-screen bg-[#020a18] text-slate-100" onPointerMove={(event) => setCursor({ x: event.clientX, y: event.clientY })} onPointerDown={() => setIsDragging(true)} onPointerUp={() => setIsDragging(false)} onPointerLeave={() => setIsDragging(false)}>
      ...
      <section id="achievements" className="stats-section"><div className="stat-card"><strong>2025</strong><span>Computer Science graduate</span></div><div className="stat-card"><strong>2+</strong><span>Freelance projects shipped</span></div><div className="stat-card"><strong>01</strong><span>Company · Skillauro</span></div></section>
      <section className="company-strip"><div><span className="section-kicker">current company</span><h2>Building at <em>Skillauro.</em></h2></div><a href="https://www.skillauro.in" target="_blank" rel="noreferrer">Visit skillauro.in <ArrowUpRight /></a></section>
